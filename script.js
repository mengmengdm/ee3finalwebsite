...
async function fetchOrders() {
    try {
        ...
        if (!Array.isArray(pendingOrders) || !Array.isArray(completedOrders)) {
        ...
        if (pendingContainer) pendingContainer.innerHTML = '<div class="text-center text-muted">Failed to load orders</div>';
        if (completedContainer) completedContainer.innerHTML = '<div class="text-center text-muted">Failed to load orders</div>';
    }
}
...
function displayOrders(pendingOrders, completedOrders) {
    ...
    if (pendingOrders.length === 0) {
        pendingContainer.innerHTML = '<div class="text-center text-muted">No pending orders</div>';
    } else {
    ...
    if (completedOrders.length === 0) {
        completedContainer.innerHTML = '<div class="text-center text-muted">No completed orders</div>';
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            if (!href) {
                console.error('Navigation link has no href attribute');
                return;
            }
